...
function GitHubStarsButton({
  ...
  const fillPercentage = Math.min(100, (motionNumberRef.current / Math.max(stars, 1)) * 100);
  const formattedResult = formatNumber(motionNumberRef.current, formatted);
  // Always show at least "0" to prevent layout jump, use actual stars when loaded
  const ghostFormattedNumber = formatNumber(stars || 0, formatted);
